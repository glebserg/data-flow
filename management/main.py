...
async def get_range_metrics(start: int, end: int, metrics_cls=Depends(get_cls_metrics),
    ...
    result = metrics_cls.select().where(
        (metrics_cls.sensor == sensor), ((metrics_cls.created > start_dt) & (metrics_cls.created < end_dt))
    ).order_by(metrics_cls.created)
    return [{metric_obj.id: {"value": metric_obj.value, "created": metric_obj.created}
             for metric_obj in list(result)}]
